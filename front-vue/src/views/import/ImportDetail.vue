  ...
  methods: {
    ...
    async save() {
      ...
      const response = await fetch(
        `http://localhost:3000/api/theImport/${this.selectedCustomerId}`,
        {
          method: "PUT",
          headers: {
  ...
  watch: {
    show: async function () {
      if (this.show) {
        ...
        this.theImportDetail = await response.json();
        this.theImportDetail.moreInfo = this.theImportDetail.moreInfo === "null" ? '' : this.theImportDetail.moreInfo;
        this.$store.action.hideLoading();
      }
    },
  },
